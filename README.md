...
*   **Для Windows:** Добавьте путь к исполняемым файлам PostgreSQL в переменную среды `PATH`. Обычно это `C:\Program Files\PostgreSQL\<НОМЕР_ВЕРСИИ>\bin` (например, `C:\Program Files\PostgreSQL\18\bin`). Это позволяет использовать команды `psql`, `pg_dump` и другие из любой директории в командной строке.
